...
QTQ.Item {
  ...
  SectionSettingsRow: QTQ_L.RowLayout
  {
    ...
    QTQ_C.Label {
      text: parent.label; QTQ_L.Layout.fillWidth: true
    }
    QTQ_C.Label {
      text: qsTr("Width")
    }
  ...
  QTQ.ListModel {
    id: sectionModel
    QTQ.ListElement {
      key: "date"; label: qsTr("Date")
    }
    QTQ.ListElement {
      key: "desktopName"; label: qsTr("Desktop name")
    }
    QTQ.ListElement {
      key: "number"; label: qsTr("Desktop number")
    }
  }
  ...
  function loadSettings() {
    ...
    for (var l = 0; l < listNames.length; ++l)
    {
      ...
      if (stored)
      {
        try
        {
          root[listNames[l]] = JSON.parse(stored)
        }
        catch (e)
        {
        }
    ...
    QTQ.Item {
      ...
      QTQ.Rectangle {
        ...
        QTQ.Text {
          ...
          QTQ.MouseArea {
            id:
                dayDateMouse; anchors.fill: parent; hoverEnabled: true; enabled: preview.interactive; onClicked: root.openStyle("dayDate")
          }
          QTQ.Rectangle {
            anchors.fill: parent
            color: "transparent"
        ...
        QTQ.MouseArea {
          id:
              dayNameMouse; anchors.top: parent.top; anchors.left: parent.left; anchors.right: parent.right; height: parent.height / 2; hoverEnabled: true; enabled: preview.interactive; onClicked: root.openStyle("dayName")
        }
      }
      QTQ.Rectangle {
        id: nameBlock
      ...
      QTQ.Rectangle {
        ...
        QTQ.MouseArea {
          id:
              numberMouse; anchors.fill: parent; hoverEnabled: true; enabled: preview.interactive; onClicked: root.openColor("number", numberBlock.color)
        }
        QTQ.Text {
          ...
          QTQ.MouseArea {
            id:
                numberTextMouse; anchors.fill: parent; hoverEnabled: true; enabled: preview.interactive; onClicked: root.openStyle("numberText")
          }
          QTQ.Rectangle {
            anchors.fill: parent
            color: "transparent"
  ...
  QTQ_C.Dialog {
    ...
    contentItem: QTQ_L.ColumnLayout
    {
      spacing: Kirigami.Units.largeSpacing
      QTQ_C.Label {
        text: qsTr("Text appearance"); QTQ_L.Layout.fillWidth: true
      }
      QTQ_C.Button {
  ...
  QTQ_D.ColorDialog {
    ...
    onAccepted: {
      if (target === "date")
      {
        root.dateBackgroundColors = root.setAt(root.dateBackgroundColors, root.selectedDesktop - 1, selectedColor);
        root.save("dateColor", selectedColor)
      }
      else if (target === "number")
      {
        root.numberBackgroundColors = root.setAt(root.numberBackgroundColors, root.selectedDesktop - 1, selectedColor);
        root.save("numberColor", selectedColor)
      }
      else if (target === "desktopNameBackground")
      {
        root.desktopNameBackgroundColors = root.setAt(root.desktopNameBackgroundColors, root.selectedDesktop - 1, selectedColor);
        root.save("desktopNameBackgroundColor", selectedColor)
      }
      else
      {
        ...
        {
          root.dayNameColors = root.setAt(root.dayNameColors, root.selectedDesktop - 1, selectedColor);
          root.save("dayNameColor", selectedColor)
        }
        else if (styleDialog.target === "dayDate")
        {
          root.daydateBackgroundColors = root.setAt(root.daydateBackgroundColors, root.selectedDesktop - 1, selectedColor);
          root.save("dayDateColor", selectedColor)
        }
        else if (styleDialog.target === "desktopName")
        {
          root.desktopNameColors = root.setAt(root.desktopNameColors, root.selectedDesktop - 1, selectedColor);
          root.save("desktopNameColor", selectedColor)
        }
        else
        {
          root.numberColors = root.setAt(root.numberColors, root.selectedDesktop - 1, selectedColor);
          root.save("numberTextColor", selectedColor)
        }
      }
    }
  }
  ...
  QTQ_C.Dialog {
    ...
    onAccepted: {
      ...
      if (target === "dayName")
      {
        root.dayNameFonts = root.setAt(root.dayNameFonts, root.selectedDesktop - 1, family);
        root.save("dayNameFont", family)
      }
      else if (target === "dayDate")
      {
        root.dayDateFonts = root.setAt(root.dayDateFonts, root.selectedDesktop - 1, family);
        root.save("dayDateFont", family)
      }
      else if (target === "desktopName")
      {
        root.desktopNameFonts = root.setAt(root.desktopNameFonts, root.selectedDesktop - 1, family);
        root.save("desktopNameFont", family)
      }
      else if (target === "numberText")
      {
        root.numberFonts = root.setAt(root.numberFonts, root.selectedDesktop - 1, family);
        root.save("numberFont", family)
      }
